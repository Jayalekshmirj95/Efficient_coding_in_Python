...
Let me first explain a little bit about the task that we were working on. The task was to create a table which contains all the historical invoices. We had around 7 source tables that we could use to create the resultant table called _Invoices_. BigQuery is the platform where these source tables were stored. From there we were able to get the data using Python (to connect with BigQuery) and then do the processing and manipulations to get the final output.
